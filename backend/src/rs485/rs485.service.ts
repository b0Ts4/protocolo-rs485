...
export class Rs485Service {
  ...
  private readonly debugEnabled = (process.env.RS485_DEBUG ?? '') === '1';
  ...
  async sendRequest(
    frame: Rs485Frame,
    responseCommand: number = frame.command
  ...
  async readHoldingRegisters(address: number, start: number, count: number): Promise<Buffer> {
    ...
    const response = await this.sendRequest({ address, command: 0x03, payload }, 0x03);
    if (response.payload.length < 1) throw new Error('Invalid Modbus response (empty payload)');
    const byteCount = response.payload[0];
    const data = response.payload.slice(1);
    if (byteCount !== data.length) throw new Error('Invalid Modbus response (byteCount mismatch)');
    if (data.length < count * 2) throw new Error('Invalid Modbus response (insufficient data)');
    return data;
  }
